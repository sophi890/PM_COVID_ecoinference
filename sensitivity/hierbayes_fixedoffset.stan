...
parameters{
  vector[numeffects[1]+numeffects[2]] pars; 
  real mu;
...
model{
  mu ~ normal(0,5); // prior for intercept. Using fixed offsets so centered at 0
  rand ~ normal(mu, sigma);
  pars ~ normal(0, sqrt(0.68)); // priors for remaining //corresponds to 95 percent odds ratio between 1/5 and 5
